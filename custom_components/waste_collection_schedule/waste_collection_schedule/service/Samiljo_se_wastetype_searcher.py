...
parser.add_argument(
    "--char", type=str, help='exclude characters from search (eg "abcdef")'
)
...
def waste_searcher(arg1):  # sourcery skip: use-fstring-for-concatenation
    ...
    # Calendar uses different tags for the last week of the month
    wastedays = soup.find_all("td", {"style": "styleDayHit"}) + soup.find_all(
        "td", "styleDayHit"
    )
...
with alive_bar(bar_count) as bar:
    for char in alphabet:
        if not used_char:
            ...
            adresslist = requests.get(API_URLS["address_search"], params={"svar": "a"})
            checked_addresses = adresslist.text.lower().splitlines()
            alphabet = "".join(alphabet.split("a", 1))

        else:
            adresslist = requests.get(API_URLS["address_search"], params={"svar": char})
...
if args.city and args.street:
    ...
    checked_addresses2.extend(
        line for line in checked_addresses if args.street.lower() in line
    )
    checked_addresses = checked_addresses2
elif args.city:
    checked_addresses2.extend(
        line for line in checked_addresses if args.city.lower() in line
    )
    checked_addresses = checked_addresses2
...
if NEW_NAME_MAP:
    ...
    print(
        f"The waste types for {args.street}, {args.city} are already included in the NAME_MAP."
    )
else:
