...
def get_waste_types(hpid, realmid):
    r = requests.get(f"http://slim.cmcitymedia.de/v1/{hpid}/waste/{realmid}/types")
    r.raise_for_status()
...
def get_waste_districts(hpid, realmid):
    r = requests.get(f"http://slim.cmcitymedia.de/v1/{hpid}/waste/{realmid}/districts")
    r.raise_for_status()
...
def get_all_hpid():
    ...
    for i in range(i_from, i_to):
        ...
        if r.status_code == 200:
            ...
            if len(items) > 1:
                print(
                    "Found more than one, aborting, look what's going on here, because this is not expected."
                )
                break
...
def get_all_districts():
    ...
    for founds in hpids:
        ...
        districts_by_hpid[hpid] = (
            founds,
            get_waste_districts(hpid, realmid),
            get_waste_types(hpid, realmid),
        )
...
def main():
    i = input(
        "What do you want to do? (1: get all hpid, 2: get waste types, 3: get waste districts, 4: generate icons list, 5: generate all icons lists, 6: generate attributes to md file): "
    )
    if i == "1":
        get_all_hpid()
    elif i == "2":
        ...
        text = gen_icons(hpid, realm)
        print(
            "Copy this into custom_components/waste_collection_schedule/waste_collection_schedule/service/CMCityMedia.py"
        )
        print("")
        print(text)
    elif i == "5":
