...
TEST_CASES = {
    "Test_001": {"uprn": "10022892379"},    # has multiple collections on same week per bin type
...
class Source:
    ...
    def fetch(self):
        ...
        # Extract bin types and next collection dates, for some reason unlike all others that use this service,
        # you need to submit a bin type to get useful dates.
        for bin_name in BIN_MAP.keys():
            ...
            for item in rowdata:
                collection_date = rowdata[item]["Date"]
                # need to dedupe as MBC seem to list the same collection twice for some places
                if collection_date not in seen:
                    entries.append(
                        Collection(
                            t=BIN_MAP[bin_name]['name'],
                            date=datetime.strptime(
                                collection_date, "%d/%m/%Y"
                            ).date(),
                            icon=BIN_MAP.get(bin_name).get('icon'),
                        )
                    )
                    # add this date to seen so we don't use it again
                    seen.append(collection_date)
