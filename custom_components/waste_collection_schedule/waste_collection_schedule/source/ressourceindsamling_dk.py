...
ADDRESS_LOOKUP_URL = (
    "https://selfserviceapi.waste2x.dk/api/Customer/Customer/SearchCustomer/"
)
...
class Source:
    ...
    def fetch(self):
        ...
        try:
            ...
            raise ValueError(
                "Failed to parse services response from waste2x.dk"
            ) from exc
        ...
        for item in data:
            entries.append(
                Collection(
                    ...
                    icon=ICON_MAP.get(
                        item["serviceTypePublicName"], "mdi:trash-can-outline"
                    ),
                )
            )
