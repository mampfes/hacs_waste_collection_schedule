...
class Source:
    ...
    def fetch(self) -> list[Collection]:
        ...
        _LOGGER.debug(
            "Requesting search page URL '%s' to create a session to avoid captcha problems", SEARCH_PAGE_URL)
        response = session.get(SEARCH_PAGE_URL)
        response.raise_for_status()

        # fetch autocomplete result
        _LOGGER.debug("Searching the autocomplete API endpoint '%s' using term '%s'...",
                      API_URL, self._street_address)
        ...
        for key, value in search_results["result"][0].items():
            ...
            if key.endswith(_DATE_STRING) and (key.startswith(_NEXT_STRING) or key.startswith(_PREV_STRING)):
                name = key.replace(_DATE_STRING, "").replace(
                    _NEXT_STRING, "").replace(_PREV_STRING, "")
