...
TEST_CASES = {
    "Residential - Villa": {
        "address": "Helsingborgsvägen 31",
        "customer_number": 25494,
    },
    "Residential - Apartment": {"address": "Hallandsvägen 9", "customer_number": 13726},
    "Commercial": {"address": "Hallandsvägen 9", "customer_number": 33159},
}
...
class Source:
    def __init__(self, address: str, customer_number: int = 0):
        self._address = address
        self._kundNr = customer_number
    ...
    def fetch(self) -> list[Collection]:
        ...
        if len(addresses) == 1:
            ...
            if self._kundNr == 0:
                # if multiple addresses are found but no customer_number is given, throw exception
                raise SourceArgumentException(
                    argument="address",
                    message=f"Multiple values found for the argument 'address' with the value '{self._address}'",
                )
            else:
                # If a customer_number is given, use the one with the matching customer_number
                ...
                if not (hsG and hsO and knR and abK and nrA):
                    # If no matching customer_number is found, raise exception
                    raise SourceArgumentException(
                        argument="customer_number",
                        message=f"No results found for the argument 'customer_number' with the value '{self._kundNr}'. Please check the value and verify the address.",
                    )
        else:
