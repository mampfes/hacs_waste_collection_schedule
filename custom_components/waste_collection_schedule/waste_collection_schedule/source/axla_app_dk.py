...
def select_test_cases(municipalities):
    ...
    for name, info in municipalities.items():
        test_cases[name] = {"municipality": name, "addressid": info["addressid"]}
...
class Source:
    def __init__(self, municipality: str, addressid: str):
        _LOGGER.debug(
            "Initializing Source with municipality=%s, addressid=%s",
            municipality,
            addressid,
        )
            ...
            for collection in entry["collections"]:
                entries.append(
                    Collection(
                        ...
                        t=str(
                            TYPE_MAP.get(
                                collection["fraction"]["name"],
                                collection["fraction"]["name"],
                            )
                        ),
                        icon=ICON_MAP.get(
                            collection["fraction"]["name"], Icons.GENERAL_WASTE
                        ),
                    )
                )
