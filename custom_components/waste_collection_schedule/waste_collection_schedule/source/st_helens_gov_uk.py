...
class Source:
    ...
    def _get_input_field_value_by_name(
        self, soup: BeautifulSoup, field_name: str
    ) -> str:
    ...
    def fetch(self):
        ...
        page_session_id = self._get_input_field_value_by_name(
            soup, "RESIDENTCOLLECTIONDATES_PAGESESSIONID"
        )
        session_id = self._get_input_field_value_by_name(
            soup, "RESIDENTCOLLECTIONDATES_SESSIONID"
        )
        nonce = self._get_input_field_value_by_name(
            soup, "RESIDENTCOLLECTIONDATES_NONCE"
        )

        _LOGGER.debug(
            "Get initial form and variables - page_session_id: %s\nsession_id: %s\nnonce: %s",
            page_session_id,
            session_id,
            nonce,
        )
