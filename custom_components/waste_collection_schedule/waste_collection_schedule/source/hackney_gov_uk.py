...
class Source:
    def __init__(self, uprn: str, postcode: str):
        ...
        if not postcode:
            raise SourceArgumentNotFound("postcode")
    ...
    def fetch(self) -> list[Collection]:
        with requests.Session() as session:
            ...
            try:
                resp = session.post(f"{API_ROOT}/property/opensearch", json=payload)
            ...
            try:
                prop_resp = session.get(f"{API_BASE}/getproperty/{system_id}")
            ...
            for c_id in container_ids:
                try:
                    bin_data = session.get(f"{API_BASE}/getbin/{c_id}").json()
                    raw_name = bin_data.get("subTitle", "Waste")
                    
                    workflow_data = session.get(f"{API_BASE}/getcollection/{c_id}").json()
                    workflow_ids = workflow_data.get("scheduleCodeWorkflowIDs", [])

                    for w_id in workflow_ids:
                        dates_resp = session.get(f"{API_BASE}/getworkflow/{w_id}")
                        ...
                        for date_str in raw_dates:
                            try:
                                ...
                                if date_obj >= datetime.date.today():
                                    ...
                                    for keyword, (friendly, icon_slug) in KEYWORD_MAP.items():
