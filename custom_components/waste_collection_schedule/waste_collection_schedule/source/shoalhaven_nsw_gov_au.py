...
URL = "https://www.shoalhaven.nsw.gov.au/Home"
TEST_CASES = {
...
class Source:
    def __init__(self, geolocation_id: str):
        """
        Initializes the Source with the provided geolocation ID.
    ...
    def fetch(self) -> list[Collection]:
        """
        Fetches the waste collection schedule from the Shoalhaven City Council API.
        ...
        if not data.get("success"):
            raise Exception(f"API call was not successful: {data.get('responseContent', 'No specific error message provided by API.')}")
        ...
        if not entries:
            ...
            raise Exception("No collection schedules found. Please check your Geolocation ID and ensure the API is returning valid data for your address.")
