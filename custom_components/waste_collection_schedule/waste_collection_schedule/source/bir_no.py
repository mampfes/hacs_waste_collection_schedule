...
class Source:
    ...
    def fetch(self):
        ...
        r = requests.get(f"{URL}/adressesoek/toemmekalender", params = {"rId":{r.json()[0]['Id']}}, headers = headers)
        doc = etree.HTML(r.content)
        month_containers = doc.cssselect('.main-content .address-page-box .month-container')

        return [
            Collection(
                date=datetime.datetime.strptime(
                    f"{date.text.replace('des','dec').replace('mai','may').replace('okt','oct')} {container.cssselect('.month-title')[0].text.strip().split(' ')[1]}", "%d. %b %Y"
                ).date(),
                t=doc.cssselect(f'.trash-categories > .trash-row > img[src="{category_row.cssselect(".icon > img")[0].get("src")}"] + .trash-text')[0].text,
                icon=map_icon(doc.cssselect(f'.trash-categories > .trash-row > img[src="{category_row.cssselect(".icon > img")[0].get("src")}"] + .trash-text')[0].text.lower())
            )
