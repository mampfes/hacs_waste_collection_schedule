...
class Source:
    def __init__(self, adresse: str, kommune: str = ""):
    ...
    def fetch(self):
        ...
        if not uuid:
            raise Exception(f"Found no valid ID for the address: {self._adresse}. Check your spelling.")

        # =========================================================
        # STEP 2: Fetch the actual calendar using the found UUID
        # =========================================================
        safe_address_param = urllib.parse.quote_plus(self._adresse)
        ...
        for item in calendar_list:
            ...
            try:
                ...
                if "restavfall" in waste_lower:
                    entries.append(Collection(date=date_obj, t="Restavfall", icon=ICON_MAP["Restavfall"]))
                    found_any = True
                
                if "matavfall" in waste_lower:
                    name = "Matavfall uten hageavfall" if "uten hageavfall" in waste_lower else "Matavfall"
                    entries.append(Collection(date=date_obj, t=name, icon=ICON_MAP["Matavfall"]))
                    found_any = True
                
                if "papir" in waste_lower:
                    name = "Papir og papp" if "og papp" in waste_lower else "Papir"
                    entries.append(Collection(date=date_obj, t=name, icon=ICON_MAP["Papir"]))
                    found_any = True
                
                if "plast" in waste_lower:
                    name = "Plastemballasje" if "emballasje" in waste_lower else "Plast"
                    entries.append(Collection(date=date_obj, t=name, icon=ICON_MAP["Plast"]))
                    found_any = True
                
                if "glass" in waste_lower or "metall" in waste_lower:
                    name = "Glass- og metallemballasje" if "metallemballasje" in waste_lower else "Glass og metall"
                    entries.append(Collection(date=date_obj, t=name, icon=ICON_MAP["Glass"]))
                    found_any = True

                # Fallback in case they add a new type of waste we haven't mapped yet
                if not found_any:
                    entries.append(Collection(date=date_obj, t=waste_original, icon=ICON_MAP["Restavfall"]))
