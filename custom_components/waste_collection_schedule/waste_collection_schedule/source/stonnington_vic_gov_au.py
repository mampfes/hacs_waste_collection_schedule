...
class Source:
    ...
    def fetch(self):
        ...
        response = session.get(
            f"https://www.stonnington.vic.gov.au/api/v1/myarea/search",
            params={'keywords': self._street_address}
        )
        response.raise_for_status()
        addressSearchApiResults = response.json()
        if addressSearchApiResults['Items'] == None or len(addressSearchApiResults['Items']) < 1:
            _LOGGER.error(f"Address search for '{self._street_address}' returned no results. Check your address on https://www.stonnington.vic.gov.au/Services/Waste-and-recycling")
            return []
        ...
        for article in soup.find_all('article'):
            ...
            icon = \
                "mdi:leaf" if waste_type == "Food and Green Waste" else \
                "mdi:sofa" if waste_type == "Hard Waste" else \
                "mdi:recycle" if waste_type == "Recycling" else \
                "mdi:trash-can"
