...
class Source:
    ...
    def fetch(self):
        ...
        if self._uprn:
            ...
            valid_options = [opt for opt in options if not opt.get_text(strip=True).startswith("Select")]
            
            if not valid_options:
                raise ValueError(f"No valid addresses found for postcode: {self._postcode}")
