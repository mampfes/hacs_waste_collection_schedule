...
TITLE = "Roslagsvatten"
DESCRIPTION = (
    "Source for Roslagsvatten waste collection (Österåker, Vaxholm and Ekerö)."
)
...
class Source:
    ...
    def fetch(self):
        ...
        r = requests.post(
            f"{self._api_url}/search", data=json.dumps(search_payload), headers=headers
        )
        r.raise_for_status()
        ...
        if (
            ...
            or "data" not in search_results[0]
        ):
            raise SourceArgumentException(
                "street_address",
                "Unexpected response format from Roslagsvatten API. Please try again later.",
            )
        ...
        fetch_payload = {"buildingId": building_id, "municipality": self._municipality}

        r = requests.post(
            f"{self._api_url}/fetch", data=json.dumps(fetch_payload), headers=headers
        )
        r.raise_for_status()
        ...
        if not entries:
            _LOGGER.info(
                f"No collection entries found for address: {self._street_address}"
            )
