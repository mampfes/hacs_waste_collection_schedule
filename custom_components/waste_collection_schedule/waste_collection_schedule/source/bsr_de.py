...
def downloadMonthlyICS(s, abf_strasse, abf_hausnr, month, year):
        ...
        s.post("https://www.bsr.de/abfuhrkalender_ajax.php?script=dynamic_kalender_ajax", data=args)
...
class Source:
    ...
    def fetch(self):
        ...
        for i in range(12):
            ...
            ics = downloadMonthlyICS(session, self._abf_strasse, self._abf_hausnr, month, year)
            dates.extend (self._ics.convert(ics) )


        if now.month in [12, 1]:
            # have to reinitialize session and address search for fetching christmas tree collection schedules, otherwise it doen't work
            sessionChristmastrees = initializeSession(self._abf_strasse, self._abf_hausnr)
            ics = downloadChristmastreeICS(sessionChristmastrees, self._abf_strasse, self._abf_hausnr)
            dates.extend (self._ics.convert(ics) )
