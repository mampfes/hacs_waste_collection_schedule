...
class Source:
    ...
    def fetch(self):
        # get json file
        r = requests.get(
            f"http://app.newark-sherwooddc.gov.uk/bincollection/calendar?pid={self._uprn}"
        )
        ...
        for month in months:
            ...
            for collection_day in rows:
                ...
                collection_day_match = re.search(r',\s*\w*\s*(\d{1,2})\w{2}', collection_day.find('td').get_text(strip=True))
                ...
                entries.append(
                        Collection(
                            ...
                            t=BINS[collection_type]['name'],  # Collection type
                            icon=BINS[collection_type]['icon'],  # Collection icon
                        )
                    )
