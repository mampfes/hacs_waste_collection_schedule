...
| `COUNTRY` | `str` | **Lowercase code** from `update_docu_links.py`'s `COUNTRYCODES` list. UK = `"uk"` (NOT `"gb"`); Canada = `"ca"` (lowercase). ISO 3166-1 alpha-2, so Slovenia is `si`. An invalid value silently orphans the source out of README/info/sources.json. |
...
**Declare `WASTE_TYPES` explicitly**, as the canonical types the source actually produces, derived by replaying its cassette. `BaseSource.__init_subclass__` will auto-derive it, but only from a transformer's explicit `type_value_map`, so it misses everything the shared vocabulary resolves, and `tests/test_declared_waste_types.py` replays every cassette and rejects that. **An empty list is acceptable for a bare transformer with no map**, where nothing can be enumerated statically; that shape used to derive the whole `ALL_TYPES` catalogue, claiming every canonical type and declaring nothing, and the fallback was removed in #7028. The list feeds the config-flow waste-type dropdown and nothing else.
...
1. **Language allowlist:** language codes are ISO 639-1, a different namespace from `COUNTRY` (Slovenia is country `si` but language `sl`). A language is only added to `LANGUAGES` together with `waste_types.SUPPORTED_LANGUAGES`, with complete translations (`waste_types` names, `field_terms` labels, `default_translations.py`); `tests/test_new_architecture.py` enforces that the two match.
   Allowlist rule: `PARAM_TRANSLATIONS` and `PARAM_DESCRIPTIONS` keys MUST be in `{"en", "de", "it", "fr", "nl", "sl", "da}` (the `LANGUAGES` list in `update_docu_links.py` is the source of truth). **Default behaviour when a contributor / agent wants to use any other language** (e.g. `fi`, `es`, `pl`): strip the unsupported-language block from the source's translation dicts in the current PR, and open a *separate* issue titled `Add <lang> (xx) language support to PARAM_TRANSLATIONS allowlist` linked back to the original PR/issue, asking for contributors to help with the full translation pipeline (allowlist + `update_docu_links.py` + `translations/<xx>.json`). Never silently include an unsupported language; CI will reject the PR.
