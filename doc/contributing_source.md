...
| HOW_TO_GET_ARGUMENTS_DESCRIPTION | Dict | [Optional] Description of how to get the arguments, will be shown in the GUI configuration form above the input fields. Keys must be language codes from `en`, `de`, `it`, `fr` — other codes will fail the test suite. You do not need to provide all four languages. |
...
| SOURCE_CODEOWNERS | List of Strings | [Optional] List of GitHub handles (for example `["@your-user"]`) who maintain this source. Each entry must start with `@`. Used by `update_docu_links.py` to generate `.github/source_owners.json`. A GitHub Action automatically pings and assigns everyone listed when a `[Source Defect]` bug report names this source — so adding your handle means you will be notified of issues and can keep your source healthy. **Strongly encouraged for new sources.** |
