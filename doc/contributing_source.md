...
| i-web `/abfalldaten` (CH) | `abfalldaten_parser()`, `AbfalldatenRows` (`area`, `expand_ranges`) | The Swiss i-web municipal CMS's collection table (`#icmsTable-abfallsammlung`), shipped as JSON in its `data-entities` attribute. `AbfalldatenRows` yields `(date, name)` rows, filtered to one collection district (`abfallkreisIds` / `abfallkreisNameList`) when the municipality has several. Adding a municipality is its `/abfalldaten` URL and a type map. |
| 3C Shared Services waste calendar (UK) | `uprn_retriever(authority=...)`, `address_retriever()`, `CollectionsParser`, `TYPE_VALUE_MAP` | `servicelayer3c.azure-api.net/wastecalendar`, shared by Cambridge, South Cambridgeshire and Huntingdonshire: a postcode address search, then one collections list per UPRN, each collection naming the rounds run that day. |
