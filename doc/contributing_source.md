...
- Source documentation markdown files in `doc/source/`
...
# NOTE: per-argument labels and descriptions are NOT defined inline in
# this file. They live in YAML next to the source module — see the
# "Argument labels and descriptions (i18n)" section below.
...
The following files are **automatically generated by CI** after your PR is merged and should **not** be manually edited or committed:

- `README.md` — the list of supported service providers
- `info.md` — rendered in the HACS user interface within Home Assistant
- Source documentation markdown files in `doc/source/`

These are generated by the `update_docu_links.py` script, which iterates through all source files and extracts meta information like title and url. It is therefore important to set the attributes in the source file correctly. By default, the country classification is derived from the file name. If this doesn't match, the country code can be overwritten with the attribute `COUNTRY`.
...
| TITLE_LANG | String | [Optional] Language code (`"en"`/`"de"`/`"fr"`/`"it"`) used to spell-check the `TITLE` string. Defaults to `"en"`. Set this for non-English titles such as `"Abfallwirtschaft Bad Kreuznach"` (`TITLE_LANG = "de"`). |
| EXTRA_INFO_LANG | String | [Optional] Same as `TITLE_LANG` but applied to every `EXTRA_INFO[].title` string in the module. Set when the listed providers have non-English names. |
...
## Argument labels and descriptions (i18n)

Argument labels and descriptions shown in the Home Assistant UI live in
YAML files, not in the Python source. Two layers feed the translation
generator:

1. **Shared registry** —
   `custom_components/waste_collection_schedule/i18n/shared/<lang>.yaml`
   already defines labels and descriptions for the common parameter
   names (`address`, `city`, `street`, `house_number`, `postcode`,
   `uprn`, ...). If your `__init__` uses these, **you don't need to
   write anything**: the labels resolve automatically across all four
   supported languages (`en`, `de`, `fr`, `it`).

2. **Per-source overrides** — when your source uses a parameter name
   not in the shared registry, or needs different wording for one
   language, create a sibling folder next to your `.py`:

   ```
   custom_components/waste_collection_schedule/waste_collection_schedule/source/
   ├── my_source.py
   └── my_source.i18n/
       ├── en.yaml          required if introducing a non-shared param
       └── de.yaml          add native language(s) for non-English sources
   ```

   ```yaml
   # my_source.i18n/en.yaml
   ---
   params:
     bin_id:
       label: Bin ID
       description: The numeric ID printed on the bin label.
     district:
       label: District
   ```

   Only fill in **what you're overriding**. Re-stating a shared label
   is auto-pruned by the redundancy audit in `tests/test_i18n.py`.

Resolution order at generation time: per-source override →
shared registry → English fallback → auto-titlecased identifier (e.g.
`house_number` → `"House Number"`).

For the full conventions (when a description earns its place,
phrase templates for error messages, spell-check details), see
...
Raise structured exceptions from
`waste_collection_schedule.exceptions` so the message can be
rendered in the user's UI language. Don't hand-write English
messages with f-strings:

```python
# Don't do this
raise Exception(f"postcode {postcode!r} not found")

# Do this — phrase templates live in i18n/phrases/<lang>.yaml,
# and HA renders the message in the user's locale.
from waste_collection_schedule.exceptions import SourceArgumentNotFound
raise SourceArgumentNotFound(argument="postcode", value=postcode)
```
...
To ensure that the source script is working as expected, it is recommended to install and run `pytest` in the `waste_collection_schedule` directory. This will run some additional tests making sure attributes are set correctly and all required files are present. Pytest does not test the source script itself.
