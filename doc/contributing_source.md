...
| `RowRelabel(rename=, vocabulary=, strip=)` | Tidy the label of `(date, key)` rows before the transformer maps it, e.g. `strip=` a trailing `"(<street>)"` group the provider appends to every ICS summary. |
| `CollapseWeeks(keys=, day=, week_start=6)` | Collapse a calendar that marks a whole week with an all-day event on each day into one row per week, dated the day the week starts or shifted to the weekday the `day` parameter names. |
| `SortRows()` | Order `(date, key)` rows by date then key, for an endpoint that returns the same events in a different order on every request. |
...
| OpenCities / MyArea (AU/NZ) | `OpenCitiesRetriever` (`address` / `address_template`, `geolocation_id`, `warm_up_url`, `page_link`, `search_fuzzy`, `strict_address_matching`, `normalise`), `OpenCitiesParser` (`date_format`, `require_date_precise`, `approximate_dates`), `OpenCitiesProjection`, `TYPE_VALUE_MAP` + `JsonTransformer` | The council-website widget behind "My Area" / "Find my bin day" pages (`/api/v1/myarea/search`, `/ocapi/Public/myarea/wasteservices`). The retriever resolves an address (or takes a geolocation id) and caches the id per source; the parser reads the reply's HTML into `{type, date, note}` records. The API gives only the *next* date per service, so `OpenCitiesProjection` optionally projects the following dates from the cadence the note states. Spread `TYPE_VALUE_MAP` into a council's `type_value_map` and add only its own wording. Adding a council is a domain, its quirks and a type map. |
...
| Jadu XFP forms (UK) | `XfpFormRetriever` (`page`, `question`, `lookup_address`, `address`, `landing_url`) | The `/xfp/form/<id>` bin-day forms many UK councils build in Jadu Continuum: read the `__token`, answer the address question (`<question>_0_0` postcode, `<question>_1_0` UPRN), get the results page. With `lookup_address` (or `address`) the postcode is posted first and the property chosen from the form's own address list, with suggestions when it is not there. The results page is council-specific; read it with `HtmlParser`, `HtmlLabelledDates` or `HtmlTextParser`. Adding a council is its form URL, page id and question id. |
| North Yorkshire bin calendar (UK) | `bin_calendar_retriever(district)`, `BinCalendarParser`, `TYPE_VALUE_MAP` | The `northyorks.gov.uk/bin-calendar` Drupal module serving all seven former North Yorkshire districts: one POST per UPRN, answered with a Drupal AJAX command list whose results page lists each date's rounds. |
| ICS feeds behind a lookup (`service/ICS.py`) | `IcsSessionRetriever` (`steps` of `select` / `extract` / `when`, `feed_url` a URL or a list of them, `argument` to blame a field when the calendar request fails), `IcsIndexRetriever` (`label` naming a link with one or several names, `argument`, `every_match`), `IcsLookupRetriever`, `IcsFeedsParser` (`labels`, `clean`, `exclude`, `dedupe`, `clip_to_until`, ...) | A provider that hands out an ICS file only after an address, zone or district has been resolved: a stateful form, a per-address key, a page listing one download per district. Prefer these to a source that scrapes and then calls `ICS().convert`. |
