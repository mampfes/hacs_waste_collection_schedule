...
- `.github/source_owners.json` (generated by `update_docu_links.py`)

A GitHub Action (`.github/workflows/notify-source-owners.yaml`) reads the `Source (if relevant)` field from bug reports and pings the configured owner(s) for that source.

Per-source owners are defined in source files via the optional `SOURCE_CODEOWNERS` variable.

`source_metadata.json` is generated by `update_docu_links.py` and should therefore not be edited manually for ownership data.
