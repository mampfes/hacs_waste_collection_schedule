...
- If the ICS link fails (for example `TooManyRedirects` / `curl: (47) Maximum (30) redirects followed`), use the [ReCollect](/doc/source/recollect_net.md) source instead, with the `place_id` and `service_id` from the same link.
