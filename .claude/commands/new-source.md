...
The workflow is the same for contributors and maintainers. Ask for, if not yet provided:
...
1. **Check the diff is clean** (no generated files). Quick check:
   ...
   - `tests/fixtures/<module>/*.json` (one cassette per `TEST_CASES` entry)
   - `doc/regions/<module>.yaml` if the source uses a registry
   ...
   ```bash
   git checkout upstream/release/3.0.0 -- <file>
   ```

2. **Create a feature branch** from the release branch (if not already on one):
   ```bash
   git fetch upstream release/3.0.0
   git checkout -b feat/add-<module> upstream/release/3.0.0
   ```

3. **Commit** the source and its cassettes:
   ```bash
...
- Target branch is `mampfes/hacs_waste_collection_schedule:release/3.0.0` (new sources go there during the 3.0.0 window). Never their fork's master.
