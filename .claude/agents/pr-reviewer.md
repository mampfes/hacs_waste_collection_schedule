...
- **Component reuse (CI-enforced):** a new or migrated pipeline source must not define a `Retriever`/`Parser` subclass or override `retrieve` / `parse` / `preprocess` / `transform`. `test_pipeline_sources_reuse_shared_components` and `test_no_new_source_local_step_overrides` enforce this; their allowlists are a debt register being worked to zero, so a PR that adds itself to one should be sent back. Ask for the behaviour to move into the shared component under `waste_collection_schedule/service/`.
