...
Before writing any retriever or parser, check whether the provider runs on a platform that already ships reusable components in `waste_collection_schedule.service` (ArcGIS, RiSKommunal AT, Abfallnavi / regio iT DE, IntraMaps, Sitepark IES DE), and check the shared YAML and EXTRA_INFO platforms (Recollect, Recycle Coach, ICS YAML, Publidata, c-trace). If your provider is covered, add it there instead.
