...
- **Declare `WASTE_TYPES` with what the source actually produces**, derived by replaying your recorded cassette. Do not rely on the auto-derivation: it reads only an explicit `type_value_map`, so it misses everything the shared vocabulary resolves, and a bare transformer with no map falls back to the whole `ALL_TYPES` catalogue, which declares nothing. `tests/test_declared_waste_types.py` fails both shapes. A `classify()`-based source has no derivation at all and must always declare.
