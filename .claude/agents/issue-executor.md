...
- The worktree starts on a temporary branch. The plan's first step creates the feature branch (`git checkout -b <branch-name> upstream/<base>`); run it exactly, including the base. If the branch already exists (e.g. the planner left a stale branch behind), report this and stop.
